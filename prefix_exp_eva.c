...
int main()
{
    ...
    for(int i=strlen(s.infix)-1;i>=0;i--)
    {
        if(isdigit(s.infix[i])>0)
        {
            ...
            b=0;
            char v;
            for(int u=0;u<strlen(tempcopy)/2;u++)
            {
                v=tempcopy[u];
                tempcopy[u]=tempcopy[strlen(tempcopy)-u-1];
                tempcopy[strlen(tempcopy)-u-1]=v;
            }
    ...
    for(int i=0;i<count;i++)//display the postfix expression
    {
    ...
    printf("Infix to Prefix Expression\n\n");
    for(int j=0;j<count;j++)//display the postfix expression
    {
        printf("%s ",s.postfix[j]);
    }
...
{
    int d=s.result[count2-1]/s.result[count2-2];
    count2=count2-2;
    s.result[count2]=d;
    count2++;
}
...
{
    int su=s.result[count2-1]-s.result[count2-2];
    count2=count2-2;
    s.result[count2]=su;
    count2++;
}
...
{
    int a=s.result[count2-1]+s.result[count2-2];
    count2=count2-2;
    s.result[count2]=a;
    count2++;
}
